import pygame
import random #untuk memilih bentuk secara random

#Inisialisasi game
pygame.font.init() #supaya dapat mmenggunakan font-font
pygame.mixer.init() #supaya dapat memilih warna
...
class button(): #menginisiasi tombol-tombol dalam game
...
#mendefinisikan objek-objek dalam game
class Piece(object):
...
def convert_shape_format(shape): #merotasi bentuk-bentuk
    #simpan posisi '0'
    positions = []
...
def valid_space(shape, grid): #untuk mendefinisikan posisi bentuk-bentuk yang valid
...
def get_shape(): #mengambil bentuk-bentuk yang telah dibuat
    global shapes, shape_colors

    #memilih shapes random
    return Piece(5, 0, random.choice(shapes))
 
#Membuat teks di tengah layar game
...
def draw_grid(surface, row, col): #memuat grid
    gx = top_left_x
    gy = top_left_y
...
def clear_rows(grid, locked): #mendefinisikan row-row yang kosong pada game
    inc = 0
...
def draw_next_shape(shape, surface): #mendefinisikan bentuk selanjutnya akan muncul
    #label
...
def draw_window(surface, grid, score): #menggambar display game
    ...
    while game_run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                game_run == False
                pygame.display.quit()
            #key press mendeteksi gerakan bentuk-brntuk tetris yang muncul
